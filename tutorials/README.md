...
- **Public-access release gate:** the validator and finetuner source repositories are private. The current candidate therefore requires a `GITHUB_TOKEN` with read access (or pre-staged authorized checkouts) and is not a self-service public tutorial. Issue #12 tracks the required immutable public distribution path for the exact pinned workers. Promotion to public-ready is blocked until a fresh user with no repository-specific authorization can acquire the same worker implementation without reimplementation or revision substitution.
- The current authorized bootstrap passes credentials through an ephemeral Git HTTP header; credentials are not printed, placed in clone URLs, persisted in Git config, or written to any export.
...
- `MODEL_CARD.md` **exists** in this repository and is part of the release documentation. The current notebook revision does not yet link it directly; this G16 `SHOULD` deviation is recorded here and should be removed when the notebook is next materially revised (the public-bootstrap change in #12 will require a new exact-revision execution record anyway).
...
1. **Public worker bootstrap — issue #12.** The real pinned validator/finetuner implementation must be retrievable immutably by a public user without repository-specific authorization.
...
Both gates are required. Passing only static CI or authorized local pre-flight does not make the notebook public-ready.
