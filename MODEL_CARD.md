...
The notebook is **standalone** under DIMER Notebook Specification 1.1 §3.6: it is generated by `tools/build_notebook.py` and carries `src/swin_classification_pipeline/pipeline.py` (the extracted in-notebook validator, in-kernel finetuner, artifact contract and `validate_inputs` / `evaluation_report` helpers), the `timm/swinv2_tiny_window8_256.ms_in1k` snapshot manifest at revision `650d02aabf05e8adbd060a739ab39e39f53da639` and the runtime pins inline, so it runs without this repository being reachable. Static source validation is `tools/validate_release_assets.py`; the clean-runtime execution record that gates promotion is `docs/release-verification.md` (the previous notebook revision's pre-flight runs in `tutorials/RELEASE_VERIFICATION.md` are history and do not carry over).
...
The upstream weights supply the pretrained backbone. This repository adds the code around the two base checkpoints: a digest-pinned allowlist (catalog) of exactly two base checkpoints, a dataset **validator** that freezes the image folder's identity, a **finetuner** that trains, publishes a content-addressed `safetensors` artifact, reloads it and evaluates it, contract validation binding both components to a pinned contract revision, and fail-closed verification scripts. The model architecture itself is used through `timm` unchanged.

#### Intended Use and Limitations

The uses envisioned during development are narrow: supervised adaptation of a general-purpose ImageNet backbone to a user's own single-label image classes.
...
Metrics were chosen for a single-label classifier: one discrete correctness measure, one likelihood measure, and an explicit statement of what is *not* estimated.
...
**Fine-tuning data** is supplied entirely by the operator and never leaves the finetuner's local filesystem; the pipeline's runtime network policy is `DENY`.
...
| Components | validator `org.valcorza.swin-classification-dataset-validator` 0.1.0 @ `52d1fd0ca9af5bfd413b0cd7cc1e3f441560fa95`; finetuner `org.valcorza.swin-classification-finetuner` 0.1.0 @ `0621a11c1cdb6749984aaaca6919086e5c2a7d65` |
