...
REPO_NAME = "swin-classification-pipeline"
NOTEBOOK_NAME = "swin_classification_colab.ipynb"
EXPECTED_PROFILE = "E2E"
EXPECTED_MODEL_ID = "timm/swinv2_tiny_window8_256.ms_in1k"
...
# Additional 40-hex revisions a document may legitimately cite: the catalog's small variant, the pinned
# validator / finetuner worker source revisions (repository README and card cite both the 0.1.1 release
# pins and the 0.1.0 card pins) and the ml-worker contract revision.
KNOWN_SHAS: frozenset[str] = frozenset(
    (
        "0c9500fcde4c689e97ff51954debae59c158af0d",  # timm/swinv2_small_window8_256.ms_in1k (catalog entry, reference only)
        "7c6e77fd72a79e7debe9ef811fc6c3227d44ce92",  # swin-classification-dataset-validator @ release 0.1.1
        "70638493d081501501506108b23c6937eff770ef",  # swin-classification-finetuner @ release 0.1.1
        "52d1fd0ca9af5bfd413b0cd7cc1e3f441560fa95",  # swin-classification-dataset-validator @ card pin 0.1.0
        "0621a11c1cdb6749984aaaca6919086e5c2a7d65",  # swin-classification-finetuner @ card pin 0.1.0
        "0f0c221222402721ee7716edf01378604cbd6ef3",  # ml-worker contract
    )
)
# Colab form gates that must default to the non-interactive sample path.
BYOD_GATES = ("USE_BYOD", "USE_BYOD_IMAGE")
# Machine-readable artifacts the notebook must write (OUT1-OUT3, DAT24, EVAL21).
EXPECTED_OUTPUTS = (
    "outputs/swin_classification_input_manifest.json",
    "outputs/swin_classification_evaluation_report.json",
    "outputs/swin_classification_result.json",
    "outputs/swin_classification_validation_predictions.csv",
)
...
CODE_MARKERS = (
    ...
    "'finetunerWorkerReleaseDigest': FINETUNER_WORKER_RELEASE_DIGEST",
)
...
MARKDOWN_MARKERS = (
    ...
    "the pipeline ships no confidence threshold",
    "**single holdout**",
    "the verdict is `not-measurable`",
    "`sample-sanity`",
    "object detection, segmentation, multi-label tagging, zero-shot or in-context classification",
    "the finetuner is **fail-closed**",
)
...
# Worker/subprocess paths the standalone carrier must not take outside the generator-owned install cell.
FORBIDDEN_OUTSIDE_MODULE_FLEET = (
    "worker.run(",
    "worker_cli(",
    "subprocess.run([",
)
# The model card keeps its front matter as YAML lists (two catalog base models); the provenance table is
# the card's "## Model details" section.
PROVENANCE_HEADING = "## Model details"
...
def validate_model_card() -> None:
    ...
    _check(
        f"base_model: {EXPECTED_MODEL_ID}" in front or f"  - {EXPECTED_MODEL_ID}" in front.split("base_model:", 1)[1],
        "MODEL_CARD.md base_model must name MODEL_ID (scalar or first-level YAML list entry)",
    )
    ...
    _check(PROVENANCE_HEADING in text, f"MODEL_CARD.md must carry a '{PROVENANCE_HEADING}' section")
...
EXTERNAL_WEIGHT_DIGESTS: dict[str, str] = {
    "7e793c2f3576d20b5f746ac5dc6b7271745d612f9ce06ccabe382c36f39f1caa": "timm/swinv2_small_window8_256.ms_in1k 0c9500f model.safetensors (catalog only)",
}
