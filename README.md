...
## Tutorial (standalone notebook)

`tutorials/swin_classification_colab.ipynb` is the `E2E` tutorial: image-folder validation → supervised SwinV2
fine-tuning **in the kernel** → content-addressed artifact publication → fresh-boundary reload →
new-image inference, with the pinned `timm/swinv2_tiny_window8_256.ms_in1k` backbone at immutable revision
`650d02aabf05e8adbd060a739ab39e39f53da639` (see `docs/WEIGHTS.md`). It is **standalone** under
DIMER Notebook Specification 1.1 §3.6: generated by `tools/build_notebook.py` from
`tools/notebook_template.py`, it carries `src/swin_classification_pipeline/pipeline.py` verbatim, the
2-file snapshot manifest and the `pyproject.toml` runtime pins, so the exported `.ipynb` needs no clone,
no package install and no access to this repository. Do not edit the notebook by hand — edit the
package or the template and run `python tools/build_notebook.py` (`--check` is enforced by
`tests/test_notebook_parity.py`, `tools/validate_release_assets.py` and CI).

The public package API the notebook exercises (offline unit tests in `tests/`):

```python
from swin_classification_pipeline import (
    SwinClassificationPipeline, generate_synthetic_sample, validate_inputs, validate_dataset,
    majority_class_baseline, evaluation_report, verify_artifact_generation,
)
pipe = SwinClassificationPipeline.from_pretrained(weights_dir="weights/swinv2-tiny-window8-256-ms-in1k")
manifest = validate_inputs("work/dataset")                      # DAT24 input manifest (raises ValidationFailure)
handoff = validate_dataset(Path("work/dataset"), Path("work/validated"), worker_release_digest=...)
result = pipe.fit("work/dataset", "work/validated", "work/out", epochs=1)   # cuda:0 only, fail-closed
fresh = SwinClassificationPipeline.from_artifact(result["generationDir"])
fresh.predict(["new.png"])                                     # argmax; uncalibrated softmax scores
```

`weights/<model-key>/dimer-base-manifest.json` is the committed snapshot manifest (paths, bytes, SHA-256);
the checkpoint itself is git-ignored and staged locally or, in the notebook, fetched from the Hub at the
immutable revision by `stage_missing_files(..., allow_download=True)` and re-hashed by `verify_snapshot`.

Static gates (also run by `.github/workflows/standalone-notebook.yml`):
...
`MODEL_CARD.md` (DIMER Model Card Specification 1.1) records intended use, factors,
...
registry (DIMER Notebook Specification 1.1, standalone carrier; the previous in-kernel
notebook revision was labelled "Notebook Spec 2.0" and its records are kept as history).
